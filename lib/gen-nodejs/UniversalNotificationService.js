...
UniversalNotificationServiceClient = exports.Client = function(output, pClass) {
    this.output = output;
    this.pClass = pClass;
    this._seqid = 0;
    this._reqs = {};
...
UniversalNotificationServiceProcessor = exports.Processor = function(handler) {
  ...
  if (this._handler.notify.length === 1) {
    Q.fcall(this._handler.notify, args.event)
      .then(function(result) {
        var result = new UniversalNotificationService_notify_result({success: result});
        output.writeMessageBegin("notify", Thrift.MessageType.REPLY, seqid);
        result.write(output);
        output.writeMessageEnd();
        output.flush();
      }, function (err) {
        var result = new UniversalNotificationService_notify_result(err);
        output.writeMessageBegin("notify", Thrift.MessageType.REPLY, seqid);
        result.write(output);
        output.writeMessageEnd();
        output.flush();
    ...
    this._handler.notify(args.event,  function (err, result) {
      var result = new UniversalNotificationService_notify_result((err != null ? err : {success: result}));
      output.writeMessageBegin("notify", Thrift.MessageType.REPLY, seqid);
      result.write(output);
      output.writeMessageEnd();
      output.flush();
